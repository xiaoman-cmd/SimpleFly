...
> 当前状态：**0.5.7**（build 17），已构建并安装到 `~/Library/Input Methods/SimpleFly.app`，bundle 已被 TextInput 服务接受。
>
...
>
> 八套命令行单测（引擎 / 引擎反查 / 标点 / 拼音键位 / 自定义短语 / 控制器 / 重码记忆 / 简繁转换）**共 454 项全通过**（0.5.4 实测：本机 arm64 与交叉 x86_64 两种架构下均全绿），见 §7、§8。
