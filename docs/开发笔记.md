...
- **0.7.3**（build 33）—— **菜单栏图标不再比别人窄**：菜单栏读的是顶层
  ...
  **也**指向了 16×16 方形 TIFF、还删掉了顶层键，于是菜单栏图标退回 16×16（系统「简体拼音」
  与鼠须管都是 22×16 pt）。改回三键同指 22×16 pt 的 `SimpleFly.pdf`，
...
| 来源 | 实测 | 换算 |
|---|---|---|
| 系统「简体拼音」的「拼」徽标 | **22×16 px** | 22×16 pt，满槽 |
| 本输入法（0.5.1 起） | **16×16 px** | 方形，窄一圈 |
...
但**实测鼠须管有**，而且它正是菜单栏图标的来源（鼠须管 `rime.pdf` = 22×16 pt）。
...
**怎么验证改对没改对**（省一次注销）：把 `SimpleFly.pdf` 的字形临时换成「一」
（`make_icon -o <新名>.pdf -t 一 ...`），装上去看菜单栏 —— 图标变成一条横杠，
就证明这个键确实在管菜单栏。
