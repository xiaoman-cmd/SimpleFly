...
- **0.8.0**（build 35；**b36 / b38 / b39 三次修「菜单里选主题点了没反应」，b41 把主题入口收成二级菜单**）—— **候选窗皮肤扩到 13 款，加菜单选款与「跟随系统亮暗」**：
  ...
  （前两次是**认领方式**错，第三次**连命令都没回来** —— 方向完全不同，见下面 b38）：
    ...
    · **b37 的改法**：不再猜形态 —— **每款主题一个独立 selector（`menuTheme_<主题名>:`），
      ...
      解析出的主题名；上限 32 KB），它会直接说明真机走的是哪种形态。
      **0.8.1 起这条日志默认关**（一次点击写一行的调试设施，日常不该常驻）：
      `defaults write com.simplefly.inputmethod.SimpleFly MenuDebug -bool YES` 打开、
      `defaults delete … MenuDebug` 关掉 —— 排查前记得先打开，否则下面那条判据不成立。
...
- **0.8.1**（build 42）—— **没有新功能，只收上一版留下的三处毛刺**：
  ① **`MenuDebug` 从默认开改成默认关**：b35~b41 排查「菜单点了没反应」时它默认开着，
  每次点菜单写一行日志 —— 排查期值钱，日常是纯噪音（写用户目录、一次点击一行）。
  现在要自己打开：`defaults write com.simplefly.inputmethod.SimpleFly MenuDebug -bool YES`。
  **代价说清楚**：「日志一行都没写」本身就是一条判据（b38 正是靠那片空白，把方向从
  「命令回来了但没认出来」翻到「命令压根没回来」）—— 默认关之后，要用这条判据得先打开再复现。
  ② **图标 PDF 改成字节可复现**：`CGPDFContext` 每次落盘都写当前时间进 `/CreationDate` 与
  `/ModDate`，还生成一个随机的 `/ID`。于是「图标内容一字未改、工作树却每次都脏」，
  `git diff --stat` 只报 `Bin 4153 -> 4155 bytes`，很容易被当成图标改动提交上去
  （`.tiff` / `.icns` 重生成是字节一致的，只有 PDF 带时间戳，所以只有它脏）。
  `make_icon.m` 现在落盘后把这两类串**等长**覆写成固定值（日期值 23 字符、ID 32 个十六进制字符），
  原地覆写不移动任何对象偏移、xref 与 `startxref` 依然有效。
  **过程中自己踩了一脚**：`kId` 第一版手滑写成 34 个字符，「等长」不成立就等于往文件里插字节，
  把 `startxref` 撑歪 —— 是那条回读校验（`CGPDFDocument` 验页数 + 页面尺寸）拦下来的。
  于是补了两条 `_Static_assert` 把长度钉死，校验不过就把原始字节写回：
  **宁可脏一次，也不产出坏图标**。
  注：`/Producer` 里的 macOS 版本号没法这样归一（长度随系统版本变），同一台机器上它是稳定的 ——
  换 macOS 大版本后这一份还会再脏一次，属预期，不是回归。
  ③ **文档口径**：README 顶部「当前状态」还停在 **0.6.1（build 22）**；图标那段也还写着
  「菜单栏同样由 `SimpleFly.tiff` 提供」—— 0.7.3 早就改回 22×16 pt 的 `SimpleFly.pdf` 了
  （见上文 0.7.3）。一并补上 `MenuDebug` 的说明行（默认值、怎么开、为什么「日志是空的」也算判据）。
